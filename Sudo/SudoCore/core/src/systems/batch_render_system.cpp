...
namespace sudo { namespace sudo_system { 
	...
	void BatchRendererSystem::Start()
	{
		...
		m_indicesOffset = 0;
		uint m_triangleIndices[TRIANGLE_INDICES_COUNT];
		for (int i = 0; i < TRIANGLE_INDICES_COUNT; i += 3)
		{
			m_indices[i + 0] = m_indicesOffset + 0;
			m_indices[i + 1] = m_indicesOffset + 1;
			m_indices[i + 2] = m_indicesOffset + 2;
		...
		m_triangleIndexBuffer = new graphics::IndexBuffer(m_triangleIndices, sizeof(m_triangleIndices));
	...
	void BatchRendererSystem::Submit(graphics::VertexData *a_vertices, uint a_vertexCount)
	{
		if (a_vertexCount == 3) 
		{
			...
			GLintptr initialOffset = m_triangleCount*(sizeof(graphics::VertexData) * 3);
